# Here are your Instructions
<img width="1920" height="926" alt="Screenshot 2026-02-01 032710" src="https://github.com/user-attachments/assets/dcca2b3b-277f-4222-a166-9bc57852d457" />
<img width="1920" height="835" alt="Screenshot 2026-02-01 032818" src="https://github.com/user-attachments/assets/c982623e-1b57-433d-aa5b-d8dd1bd76030" />
<img width="1920" height="856" alt="Screenshot 2026-02-01 032804" src="https://github.com/user-attachments/assets/98b41442-a3ca-46cf-92d0-413d2d062a3b" />

🏋️‍♂️ Health Tracker Web App

Health Tracker is a modern fitness web application that helps users track their workouts, nutrition, and overall fitness progress in one place.
The app features secure authentication, a clean UI, and real-time data handling.

🚀 Key Features

🔐 Authentication using Google & GitHub (Firebase Auth)

🏋️ Log and manage Workouts

🍎 Track Nutrition & Calories

📊 View fitness Progress

⚡ Fast & responsive UI

☁️ Deployed on Vercel

🛠️ Tech Stack

React.js

Firebase (Auth + Firestore)

Vercel for deployment

Framer Motion for animations

🌐 Live Demo

👉 https://health-tracker-kqvb-e3c9il4xn.vercel.app/
